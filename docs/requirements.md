...
| 알려진 한계 | SMS 실채널 연동은 범위 밖(설계만) — `correlate_multichannel_signals` 툴로 합성 이벤트를 수동 주입해 검증하는 것으로 대체. Google Safe Browsing은 코드/폴백 경로만 준비됐고 실제 API 키로의 실측 검증은 아직 안 함 — 키가 없으면 자동으로 이 검사만 건너뛴다. email(Gmail API)은 의도한 테스트 계정으로 OAuth→폴링→판정까지 전부 재검증 완료(2026-09-02) — 첫 시도에서 계정 선택 실수로 실제 메일 약 100통이 읽음 처리됐다가 즉시 복구된 사고가 있었고, 이후 처리 상태 추적을 UNREAD 라벨 제거에서 전용 라벨 추가 방식으로 재설계한 뒤 올바른 계정으로 재검증해 UNREAD가 안 바뀜을 직접 확인함(`docs/design.md` 7장 참고) |
