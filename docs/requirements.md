...
| 검증 근거 | `apps/mcp-server`: `test_entity_extraction.py`, `test_multichannel_correlation_service.py`, `test_call_analysis_correlation.py`, `test_postgres_channel_signal_repository.py`, `test_rest_correlate_endpoint.py`, `test_multichannel_synthetic_scenarios.py`(합성 시나리오 4건), `test_google_safe_browsing_adapter.py`(httpx.post monkeypatch로 요청/응답/폴백 검증). `apps/api`: `test_entity_extraction.py`, `test_analyze_call_correlation.py`(N-03 마스킹 전/후 엔티티 추출 회귀 가드 포함) |
| 알려진 한계 | sms/email 채널의 실제 유입 경로(SMS 수신, Gmail API 등)는 범위 밖 — `correlate_multichannel_signals` 툴로 합성 이벤트를 수동 주입해 검증하는 것으로 대체. Google Safe Browsing은 코드/폴백 경로만 준비됐고 실제 API 키 발급(사용자 본인 Google 계정 필요)과 그 키로의 실측 검증은 아직 안 함 — 키가 없으면 자동으로 이 검사만 건너뛴다 |
