...
| 항목 | 내용 |
|---|---|
| 설명 | 통화/문자 텍스트에서 기관사칭·공포조성·긴급송금유도·개인정보요구·원격제어앱 설치유도 패턴을 탐지한다 |
...
| 수용 기준 | ① 5개 카테고리(기관사칭/공포조성/긴급송금유도/개인정보요구/원격제어앱 설치유도) 중 텍스트에 해당하는 패턴을 최소 1개 이상 탐지한다. ② 정상 대화 텍스트(예: "오늘 점심 메뉴는...")에서는 패턴을 탐지하지 않는다(오탐 없음). ③ Ollama가 죽어 있어도 요청이 실패하지 않고 규칙 기반 결과를 반환한다 |
| 구현 현황 | 완료 — v1(규칙)→v2(LLM, `360ffde`). 합성 데이터셋 26건으로 검증(`767eac1`, 정상 통화 오탐 0건). 2026-09-18 신종수법(원격제어 앱/가짜 검찰·경찰 앱 설치 유도, 가족 사칭 딥보이스콜) 대응으로 `REMOTE_CONTROL_APP` 카테고리 추가 — 합성 데이터셋 29건으로 재검증(추가 3건 오탐 없음 확인) |
