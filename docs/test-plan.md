...
사고로 파이프라인 자체(OAuth→폴링→F-01/F-02 판정)가 실제로 작동함은 확인됐고,
**재설계 이후 의도한 테스트 계정으로도 재검증 완료**(2026-09-02) — 테스트
사용자 미등록 시 Google이 자체적으로 403으로 막아주는 것까지 확인했고,
`getProfile`로 연결 계정을 먼저 확인한 뒤 폴링 실행 → 메일 2건을 F-01/F-02로
...
| 크로스채널 상관관계(우선순위 2) — SMS 실채널 미연동 | 유료 SMS 게이트웨이가 필요해 설계만 함(`docs/design.md` 7장). email(Gmail API)은 실제 계정으로 재검증 완료 |
